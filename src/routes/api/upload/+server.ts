...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!file || !(file instanceof File)) {
        return json({ success: false, message: "File tidak ditemukan atau tidak valid." }, { status: 400 });
    }
    ...
    return json({ success: false, message: err.message || "Proses upload gagal." }, { status: 400 });
  }
};